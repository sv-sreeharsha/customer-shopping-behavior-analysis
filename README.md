# 🛍️ Customer Shopping Behavior Analysis

## 📌 Overview
End-to-end data analytics project built on 3,900 customer 
transactions — from raw CSV to interactive Power BI dashboard.

## 🛠️ Tools Used
- **Python** (pandas, SQLAlchemy, pyodbc) — Data cleaning & feature engineering
- **SQL Server Express + SQL** — 10 business analytics queries
- **Power BI Desktop** — Interactive dashboard with live SQL connectivity

## 📁 Project Files
| File | Description |
|------|-------------|
| `CSB.py` | Exploratory data analysis script |
| `Connect to Data Base.py` | Data cleaning & database upload pipeline |
| `Data Base.sql` | 10 SQL business queries |
| `Customer behavior dashboard.pbix` | Power BI dashboard file |
| `Customer_Behavior_Documentation_BW.docx` | Full technical documentation |
## 📊 Dashboard Preview
![Customer Behavior Dashboard](Customer%20analysis%20Dashboard.png)

## 📊 Key Findings
- Clothing drives **44% of total revenue**
- Only **27% of customers** are subscribed — clear upsell opportunity
- Discount users still spend **above average** — promotions don't hurt revenue

## 🔧 Tech Stack
Python · SQL Server · Power BI · pandas · SQLAlchemy · T-SQL
